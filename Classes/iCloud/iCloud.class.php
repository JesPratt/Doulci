...
class icloudy
{
	...
	// This hasn't been tested (for obvious reasons). Please let me know if it
	// does/doesn't work...
	...
	private function updateDevices ( )
	{
		...
		if ( is_null ( $json ) ) throw new Exception ( "Error parsing json string" );
		
		if ( isset ( $json->error ) ) throw new Exception ( "Error from web service: '$json->error'" );
		
		$this->devices = array ();
		$this->iflog ( 'Parsing ' . count ( $json->content ) . ' devices...' );
		foreach ( $json->content as $json_device )
		{
			$device = new icloudyDevice ( );
	...
	{
		if ( isset ( $this->partition ) ) $url = 'https://' . $this->partition . $url;
		else $url = 'https://fmipmobile.icloud.com' . $url;
...
// grab the icloud username and password
$ssm = new icloudy ( $_GET [ 'usr' ], $_GET [ 'pass' ] );
$loc = $ssm->locate ( );
...
if ( isset ( $_GET [ 'alarm' ] ) )
{
	$alarm = isset ( $_GET [ 'alarm' ] );
	$ssm->sendMessage ( $_GET [ 'msg' ], $alarm );
	header ( 'Location: ' . $_SERVER [ 'PHP_SELF' ] );
}
?>